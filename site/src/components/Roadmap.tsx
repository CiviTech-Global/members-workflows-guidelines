    ...
    <div className="relative">
      ...
      <div className="space-y-6">
        {levels.map((level, index) => (
          ...
          >
            <div className="relative z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-slate-700 bg-slate-900 text-xs font-bold text-cyan-400 shadow-lg">
              {index + 1}
            </div>
            <div className="flex-1 rounded-2xl border border-slate-800 bg-slate-950/50 p-4">
              <div className="mb-1 flex flex-wrap items-center gap-2">
                <h4 className="text-base font-semibold text-slate-100">{level.title}</h4>
                <span className="rounded-full bg-slate-800 px-2 py-0.5 text-xs text-slate-400">
                  {level.timeframe}
                </span>
              </div>
              <p className="mb-1 text-sm font-medium text-cyan-400">{level.level}</p>
